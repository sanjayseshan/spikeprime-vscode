...
function timeInterval(scheduler = async_1.async) {
    return (source) => defer_1.defer(() => {
        ...
        scan_1.scan(({ current }, value) => ({ value, current: scheduler.now(), last: current }), { current: scheduler.now(), value: undefined, last: undefined }), map_1.map(({ current, last, value }) => new TimeInterval(value, current - last)));
    });
}
