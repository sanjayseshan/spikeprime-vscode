...
function activate(context) {
    ...
    let disposable = vscode.commands.registerCommand('spikeprime.upload', () => __awaiter(this, void 0, void 0, function* () {
        ...
        const slot = yield vscode.window.showInputBox({ prompt: 'Which slot should I upload to? (1-19)' });
        const serial = yield vscode.window.showInputBox({ prompt: 'Which port is the hub on? (e.g. COM25, etc.)' });
    ...
    let disposable3 = vscode.commands.registerCommand('spikeprime.delete', () => __awaiter(this, void 0, void 0, function* () {
        ...
        var _c;
        // Display a message box to the user
        const slot = yield vscode.window.showInputBox({ prompt: 'Which slot should I delete? (1-19)' });
        const serial = yield vscode.window.showInputBox({ prompt: 'Which port is the hub on? (e.g. COM25, etc.)' });
        const terminal = vscode.window.createTerminal(`Ext Terminal #${NEXT_TERM_ID++}`);
    ...
    let disposable4 = vscode.commands.registerCommand('spikeprime.start', () => __awaiter(this, void 0, void 0, function* () {
        ...
        var _d;
        // Display a message box to the user
        const slot = yield vscode.window.showInputBox({ prompt: 'Which program slot should I start? (1-19)' });
        const serial = yield vscode.window.showInputBox({ prompt: 'Which port is the hub on? (e.g. COM25, etc.)' });
        const terminal = vscode.window.createTerminal(`Ext Terminal #${NEXT_TERM_ID++}`);
    ...
    let disposable6 = vscode.commands.registerCommand('spikeprime.move', () => __awaiter(this, void 0, void 0, function* () {
        ...
        var _f;
        // Display a message box to the user
        const slot = yield vscode.window.showInputBox({ prompt: 'Which program slot should I move? (1-19)' });
        const slot2 = yield vscode.window.showInputBox({ prompt: 'Which program slot should I move it to? (1-19)' });
        const serial = yield vscode.window.showInputBox({ prompt: 'Which port is the hub on? (e.g. COM25, etc.)' });
        const terminal = vscode.window.createTerminal(`Ext Terminal #${NEXT_TERM_ID++}`);
